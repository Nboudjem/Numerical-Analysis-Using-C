...
{
/* 
   Cette fonction d�compose une matrice $A$ carr�e, r�elle, sym�trique,
   d�finie positive, en un produit $L.L^t$, avec $L$ triangulaire inf�rieure.
   Les �quations sont:
	${\rm\ pour\ } i=1,\dots,n :$
	1.	$L_{ij} = 0 {\rm\ si\ } j>i $
...
*/
	...
	for(i=1; i<=n ; i++)
		for (j=i+1; j<=n ; j++)
			mat(L,n,i,j) = 0. ;/* des z�ros dans la partie sup�rieure de L */
	for(i=1; i<=n ; i++){
		for (j=1; j<i ; j++){
			/* Calcul de L_ij */
...
int main(){
	...
	for(i=1; i<=8 ; i++)
		for (j=i+1; j<=8 ; j++)
			mat(C,8,i,j) = mat(C,8,j,i) ;/* On sym�trise C */
