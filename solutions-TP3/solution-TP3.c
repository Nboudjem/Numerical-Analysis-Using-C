...
int main(){
	...
	double a_prime[3]; /* Nous d�clarons les vecteurs a' , b' , et c' */
	double b_prime[4];
	double c_prime[8];
	...
	*/
	{
		double At[9], A_At[9]; /* Nous d�clarons les matrices: At est la matrice transpos�e */
		int i,j;
		for(i=1;i<=3;i++){
	...
	/* ***************************************************************************/
	/* Nous r�solvons uniquement " A.x = a ". Le m�me code peut �tre
	   utilis� pour r�soudre  B.x = b   et  C.x = c
	*/
	{
	double x[3];
	...
	afficher_matrice(III,8); /* resultat: III tr�s proche de l'identit�
	...
	/* ***************************************************************************/
		/* r�ponses � la question 7 . VERSION 2. */
	/* ***************************************************************************/
	{ 
	double CCC[64], III[64];
